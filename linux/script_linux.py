...
if shutil.which("git") is None:
    ...
    if install_git == 'yes':
        ...
        print("Exiting script since Git is required.")
        exit()

# check if Python3 is installed
if shutil.which("python3") is None:
    print("Python3 is not found on your system.")
    install_python3 = input("Do you want to install Python3? (yes/no): ").strip().lower()
    if install_python3 == 'yes':
        os.system("sudo apt-get update")
        os.system("sudo apt-get install python3")
    else:
        print("Exiting script since Python3 is required.")
        exit()

def check_cuda():
    if torch.cuda.is_available():
        print("CUDA is available. Training will use GPU.")
        logging.info("CUDA is available. Training will use GPU.")
        return True
    else:
        print("CUDA is not available. Training will fall back to CPU.")
        logging.info("CUDA is not available. Training will fall back to CPU.")
        return False
...
def train_yolov5(data_yaml, pretrained_weights, batch_size, epochs, img_size, use_gpu=True):
    start_time = time.time()
    print(f"Training YOLOv5 with batch size {batch_size} for {epochs} epochs...")
    device_flag = "--device 0" if use_gpu else "--device cpu"
    cmd = f"python3 yolov5/train.py --img {img_size} --batch {batch_size} --epochs {epochs} --data {data_yaml} --weights {pretrained_weights} {device_flag}"
    print("Running command:", cmd)
    os.system(cmd)
    
    train_dir = "yolov5/runs/train" # check if the 'runs/train' directory exists
...
def predict(data_yaml, weights, batch_size, epochs, detect_folder):
    ...
    cmd = f"python3 yolov5/detect.py --source dataset/valid/images --weights {weights} --conf 0.25 --img-size 640 --save-txt --name {detect_folder}"
    os.system(cmd)
